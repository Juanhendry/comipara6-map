  ...
  if (userCount === 0) {
    ...
    if (existsSync(usersPath)) {
      ...
      const insertMany = db.transaction((rows) => {
        for (const u of rows) {
          const raw = u.password || "user123";
          // Skip re-hashing if password is already a bcrypt hash (exported from Supabase)
          const isAlreadyHashed = typeof raw === "string" && raw.startsWith("$2");
          const hash = isAlreadyHashed ? raw : bcrypt.hashSync(raw, 10);
          insert.run(
            u.name || "",
            (u.email || "").toLowerCase(),
            hash,
